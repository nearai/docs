...
When using [direct completions endpoints](/cloud/private-inference#direct-completions) (e.g., `https://deepseek-v31.completions.near.ai`), your requests go straight to the model's TEE. This means only **model verification** is needed — there is no gateway in the path, so [gateway verification](/cloud/verification/gateway) can be skipped entirely.
:::
