...
### Streaming is Required for Responses API

When using E2EE with the `/v1/responses` endpoint, streaming mode must be enabled (`stream: true`). Non-streaming mode is not supported with encryption because encrypted chunks cannot be concatenated.
