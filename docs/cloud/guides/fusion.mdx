...
|-------|----------|-------------|
| `analysis_models` | Yes* | Panel models to ask before synthesis. Required unless the deployment configures `FUSION_DEFAULT_ANALYSIS_MODELS`. Model names may include or omit a leading `~`; NEAR AI strips it before direct endpoint lookup. |
| `model` | No | Judge model. Defaults to the model of the first successful panel result when omitted. |
