...
```mermaid
flowchart LR
    A["Your Machine"] -->|"🔒 HTTPS"| B["NEAR AI Cloud gateway"] -->|"🔒 HTTPS"| C["Model TEE → LLM"]
...
Self-hosted models also have direct completions endpoints at `https://{slug}.completions.near.ai/v1`, which skip the Gateway. They are experimental and have known verification limitations, so we don't recommend them for new integrations or production verification. See [Direct Completions](/cloud/experimental/direct-completions).
