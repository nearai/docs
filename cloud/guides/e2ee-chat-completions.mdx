...
```bash
curl "https://cloud-api.near.ai/v1/attestation/report?model=zai-org/GLM-5.1-FP8&signing_algo=ed25519" \
...
response = requests.get(
    ...
    headers={"Authorization": f"Bearer {os.environ['NEAR_AI_CLOUD_API_KEY']}"},
    params={
        "model": "zai-org/GLM-5.1-FP8",
...
For Cloud API, the model key is in `model_attestations[0].signing_public_key`. For a direct completions endpoint, it is in the root report's `signing_public_key`.
...
You can verify the model's public key is authentic by checking the attestation report against hardware attestation. Choose the matching endpoint flow in [Verification](/cloud/verification).
...
<Accordion title="ECDSA encryption uses SECP256K1 ECDH + HKDF-SHA256 + AES-256-GCM with 64-byte (128 hex) public keys.">
  ...
  ```bash
  curl "https://cloud-api.near.ai/v1/attestation/report?model=zai-org/GLM-5.1-FP8&signing_algo=ecdsa" \
    -H 'Authorization: Bearer <YOUR-NEAR-AI-CLOUD-API-KEY>'
  ```
...
- [Verification](/cloud/verification) — Select Cloud API or direct model attestation verification
