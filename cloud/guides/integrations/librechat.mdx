...
`titleModel` is called once per conversation. Pointing it at a reasoning model means paying for a reasoning pass on every title, so this config uses `google/gemini-2.5-flash-lite` — a cheap non-reasoning model — and lists it under `models.default` so LibreChat can select it. Set `titleModel: 'current_model'` instead if you would rather not add a second model.
