...
<Warning>
  **Experimental.** Direct completions endpoints are not recommended for new integrations or production verification workflows. Use the NEAR AI Cloud Gateway (`https://cloud-api.near.ai/v1`) instead.
  ...
  - **Signature lookups can return `404`.** Each instance keeps response signatures in its own memory. `GET /v1/signature/{chat_id}` can reach a different instance than the one that served the completion, and clients can't choose the instance.
</Warning>
