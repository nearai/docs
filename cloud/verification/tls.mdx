---
title: "Verify TLS Termination Inside the TEE on NEAR AI Cloud"
sidebarTitle: "TLS Verification"
description: "Prove your HTTPS connection terminates inside a Trusted Execution Environment by binding the TLS certificate fingerprint to the TEE attestation report."
---

NEAR AI Cloud runs inference models inside **Confidential VMs (CVMs)** — virtual machines backed by Intel TDX hardware that provide a Trusted Execution Environment (TEE). A key property of this design: even the cloud operator cannot see or tamper with what runs inside the TEE.

TLS attestation proves that the TLS connection used for your HTTPS requests terminates directly **inside the TEE**, ensuring your messages remain encrypted end-to-end — from your machine all the way into the hardware enclave.

<Note>
Check the [NEAR AI Cloud Verifier](https://github.com/nearai/nearai-cloud-verifier/tree/main) for a reference implementation of TLS attestation verification.
</Note>
...
TLS attestation cryptographically binds the server's **TLS certificate** to the **TEE hardware attestation**:
...
<Note>
**Opt-In Behavior:** `include_tls_fingerprint` is disabled by default so existing attestation clients keep the original `report_data` layout. Without the flag, `report_data[0:32]` contains the signing address bytes (padded or truncated to 32 bytes). With the flag, `report_data[0:32]` becomes `SHA256(signing_address || tls_cert_fingerprint)`.
</Note>

### Trust Model

When you perform TLS attestation verification, you trust:
...
```mermaid
flowchart TD
    A["Intel TDX Hardware"] --> Q["TDX Quote\n(signed by CPU)"]
    B["NVIDIA GPU Hardware"] --> G["GPU Attestation\n(NVIDIA NRAS)"]
    Q --> C["mr_config_id\n↓\nDocker Compose file\n↓\nContainer images"]
    Q --> D["report_data[0:32]\n↓\nSHA256(signing_address || tls_cert_fingerprint)"]
    Q --> E["report_data[32:64]\n↓\nnonce (freshness)"]
    D --> F["Live TLS cert match\n↓\nYour HTTPS connection"]
```
...
TARGET_DOMAIN = "glm-5-1.completions.near.ai"
print(f"\nVerifying: {TARGET_DOMAIN}")
```

---

### Step 2 — Fetch Attestation and Live SPKI Hash
...
### Step 4 — Verify Report Data Binds Signing Key and TLS Cert
...
### Step 5 — Verify Live TLS Certificate Matches Attested Fingerprint
...
### Step 6 — Verify Model Name

The attestation report's `model_name` field is self-reported by the inference proxy running inside the TEE. Since it originates from within the attested enclave, it tells you which model is actually serving your requests.
...
- [Model Verification](/cloud/verification/model) — Verify GPU and CPU attestation for a model TEE
