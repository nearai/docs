...
Before sending the completion, request and verify gateway evidence with a client-generated nonce. To support `provider_tee`, also request NEAR model evidence with `model=<MODEL_ID>` and `provider=near`. Reject a missing or empty `model_attestations[]`; verify every returned candidate and retain the verified results.
