...
description: "Bind a live Cloud API TLS connection to the Cloud API gateway attestation."
---
...
4. Reuse the same open connection to request `/v1/attestation/report` with `include_tls_fingerprint=true`, the nonce, and the signing algorithm you require. Include the normal `Authorization: Bearer <YOUR_NEAR_AI_CLOUD_API_KEY>` header; Cloud API report retrieval requires an API key.
