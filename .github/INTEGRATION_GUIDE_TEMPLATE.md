...
This file is a source note for future integration guide authors. It lives under
`.github/` on purpose: Mintlify publishes every `.md`/`.mdx` file in the content
tree, so do not copy it into `cloud/guides/integrations/` or create a
`_template.mdx` there. New guides go in `cloud/guides/integrations/<tool>.mdx`
and must be added to the `Integrations` group in `docs.json`.
